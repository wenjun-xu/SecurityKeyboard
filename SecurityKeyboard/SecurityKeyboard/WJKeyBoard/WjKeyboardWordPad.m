...
@implementation WjKeyboardWordPad
...
- (NSArray *)WORDArray {
    ...
    if (!_WORDArray) {
        //随机数从这里边产生
        NSMutableArray *startArray=[[NSMutableArray alloc] initWithObjects:@"【",@"】",@"{",@"}",@"#",@"%",@"^",@"*",@"+",@"=",@"_",@"!",@"\\",@"|",@"~",@"《",@"》",@"$",@"&",@".",@"...",@",",@"^_^",@"?",@".",@"'",nil];
        //随机数产生结果
        NSMutableArray *tmpArr=[[NSMutableArray alloc] initWithCapacity:0];
        //随机数个数
        NSInteger m=26;
...
- (void)addControl {
    ...
    [trasitionWordBtn setTitle:@"#+=" forState:UIControlStateNormal];
    [trasitionWordBtn setTitle:@"123" forState:UIControlStateSelected];
    [trasitionWordBtn setBackgroundColor:[UIColor colorWithRed:170/255.0 green:176/255.0 blue:186/255.0 alpha:1]];
    trasitionWordBtn.layer.cornerRadius = 5;
    trasitionWordBtn.layer.masksToBounds = YES;
    [trasitionWordBtn setTitleColor:[UIColor blackColor] forState:UIControlStateNormal ];
    trasitionWordBtn.titleLabel.font = [UIFont boldSystemFontOfSize:15];
    ...
    [numPadCheckBtn setTitle:@"拼音" forState:UIControlStateNormal];
//    numPadCheckBtn.titleLabel.font = [UIFont boldSystemFontOfSize:17];
